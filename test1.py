"""
Test script for BEV-U-Net forest row orientation estimation.

This script corresponds to the evaluation of the BEV-U-Net model reported in
Section 5.2 of the paper, including experiments on the original point clouds
and the occlusion scenarios described in Section 5.5 (10×10, 20×20, and 30×30).

To run different experiments, only the following paths need to be modified:
    - pcd_dir
    - json_dir
    - ckpt_path
...
Section 5.2 (original point clouds) and Appendix A (ablation experiments)(Full model and w/o histogram aggregation)
    pcd_dir = "val_data/val/pcd"
    json_dir = "val_data/val/json"
    ckpt_path = "model/1_original_model.pth"
...
class ForestDirVal(Dataset):
    ...
    def load_angle_uv_map(self, json_path, H, W):
        ...
        rel_deg = adjust_angle(float(forest_deg_raw), float(yaw_deg_raw))
        u, v = deg2uv(rel_deg)
