"""
Test script for the ablation study reported in Appendix A of the paper.

This script evaluates different variants of the BEV-U-Net model used in the
ablation experiments. The code itself does not need to be modified. To run
different ablation settings, only the following paths need to be updated:
...
Note:
The loss function used during training does not affect the testing results in
this script, since the evaluation only relies on the model predictions.

Ablation experiment settings:

Plain baseline and Only histogram aggregation
    pcd_dir = "val_data/val/pcd"
    json_dir = "val_data/val/json"
    ckpt_path = "model/4_ablation_plain.pth"

w/o sign-invariant loss and Only TV regularization
    pcd_dir = "val_data/val/pcd"
    json_dir = "val_data/val/json"
    ckpt_path = "model/5_ablation_wo_sign-inv.pth"

w/o TV regularization and Only sign-invariant loss
    pcd_dir = "val_data/val/pcd"
    json_dir = "val_data/val/json"
    ckpt_path = "model/6_ablation_wo_tv.pth"

The results for the Full model and w/o histogram aggregation can be obtained
using test1.py.

Notes on evaluation metrics:

    Pixel MAE and Pixel Acc
        Results computed using histogram aggregation.

    Frame Err and Frame Acc
        Results computed without histogram aggregation.
"""
...
class ForestDirVal(Dataset):
    ...
    def load_angle_uv_map(self, json_path, H, W):
        ...
        # fixed to 0 in the ablation experiments
        forest_rad = 0.0
